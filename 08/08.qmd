...
In R gibt es mehrere Pakete, mit denen man Daten grafisch darstellen kann. Grafiken, welche mit unterschiedlichen Paketen erstellt wurden, lassen sich aber nicht (einfach) miteinander kombinieren. Deshalb entscheidet man sich typischerweise vor der Erstellung einer Grafik für das zu verwendende Grafikpaket. In diesem Seminar werden wir `graphics` und `ggplot2` näher kennenlernen.
...
Eine der wichtigsten Funktionen im Base-Plotting-System ist `plot()`. Diese Funktion erstellt spezielle Visualisierungen in Abhängigkeit von den darzustellenden Daten.

Wenn man einen numerischen Vektor übergibt, erhält man eine Punktgrafik. Auf der x-Achse wird der Index der Datenpunkte dargestellt (also die Zeile des jeweiligen Datenpunktes im Data Frame):
...
Die Funktion `paste()` klebt dabei mehrere Character-Vektoren elementweise zu einem einzigen Vektor zusammen (standardmäßig wird zwischen den Elementen ein Leerzeichen eingefügt):

```{r}
paste("Hello", "World")
```

Das Ergebnis des zusammengeklebten Character-Vektors sieht also so aus:

```{r}
paste(airquality$Month, airquality$Day, "1973")
```

Dieser Vektor kann dann mit der Funktion `as.Date()` mit dem Argument `format="%m %d %Y"` in einen Datumsvektor konvertiert werden, welchen wir dann der Spalte `date` im Data Frame zugewiesen haben:
...
Möchte man einen Faktor plotten, wird automatisch eine Balkengrafik mit den Häufigkeiten der einzelnen Stufen erzeugt:
...
Mit Hilfe eines Histogramms kann man sich die (ungefähre) Verteilung der Werte eines Vektors anzeigen lassen. Die Anzahl der Säulen im Histogramm kann man mit dem Argument `breaks` explizit festlegen:
...
Eine weitere Möglichkeit die Verteilung von Werten zu visualisieren bietet die Funktion `boxplot()`. Ein Boxplot zeigt den Median, den Interquartilsabstand (IQR) sowie das Minimum und das Maximum (sowie eventuelle Ausreißer) der Daten.

```{r}
boxplot(airquality$Temp)
```

Ein einzelner Boxplot ist für sich noch relativ wenig hilfreich. Wenn man statt eines Vektors eine sogenannte *Formel* angibt, kann man mehrere Boxplots in einer Grafik darstellen. Im folgenden Beispiel werden Boxplots für `airquality$Temp` in Abhängigkeit von `airquality$Month` dargestellt:

```{r}
boxplot(airquality$Temp ~ airquality$Month)
```

Eine Formel in R wird durch das Tilde-Zeichen (`~`) definiert. Viele Funktionen verlangen Formeln als Argumente, wir werden dies vor allem bei der Berechnung von linearen Modellen intensiv nutzen.
...
Die exakt selbe Zeile Code wie vorher produziert nun eine rote Grafik:

```{r}
plot(sin(seq(0, 2*pi, length.out=50)), type="o", xlab="", ylab="")
```

Setzen wir die Farbe aber wieder auf schwarz zurück bevor wir weitere Grafiken erzeugen:
...
Mit dem Base-Plotting-System kann man zuerst eine Grafik erstellen und danach zusätzliche grafische Elemente hinzufügen. Dazu verwendet man entsprechende Funktionen, die im Folgenden vorgestellt werden.
...
Hier übergibt man die Linie in Form einer Geradengleichung, die man mit der Funktion `lm()` berechnen kann – mehr zu dieser Funktion werden wir später bei den Regressionsmodellen kennenlernen. Im Moment reicht es zu wissen, dass diese Funktion eine Formel der Form `abhängige Variable ~ unabhängige Variable` erwartet, d.h. man gibt die diesen Variablen entsprechenden Spaltennamen an.
...
abline(lm(Ozone ~ Temp, airquality), col="gray", lwd=2)
