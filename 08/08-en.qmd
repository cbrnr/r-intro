...
:::{.callout-note}
The `breaks` argument is only a recommendation if you provide a single number – the actual number of bins is adjusted to keep the plot readable. However, you can provide a vector of breakpoints to define the bin edges explicitly (for example, `breaks=seq(0, 200, by=20)` will use exactly these breaks).
:::
