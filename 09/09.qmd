...
Oft ist es interessant zu fragen, ob zwei Variablen eine gegenseitige Abhängigkeit zeigen. Man möchte also wissen, ob sich die beiden Größen ähnlich verhalten oder nicht – wenn die eine Variable zunimmt, nimmt dann die andere auch zu? Oder ist es genau umgekehrt und sie nimmt dann immer ab? Die Korrelation ist ein einfaches und populäres Maß, um diese Fragestellungen zu beantworten. Aber Achtung: Korrelationen messen *keine* kausalen Zusammenhänge, sondern nur die Stärke eines Zusammenhangs zwischen zwei Variablen.
...
Die Varianz beschreibt, wie stark die Datenpunkte um ihren Mittelwert variieren (streuen). Dementsprechend ist die Kovarianz zwischen zwei Variablen $x$ und $y$ definiert als
...
Wir können diese sieben Datenpunkte in einem Streudiagramm visualisieren:
...
$$r = \frac{24.\dot{3}}{\sqrt{24.\dot{6} \cdot 36}} \approx 0.8166$$
...
Es ist hilfreich, die Daten zuerst einmal grafisch darzustellen. Für den Zusammenhang zwischen zwei Variablen bietet sich ein Streudiagramm (inklusive Regressionsgeraden) an – so kann man die Korrelation direkt visualisieren. Wir können eine solche Grafik zunächst einmal separat für alle drei Kombinationen erzeugen:
...
:::{.callout-tip}
Bei mehreren Variablen (wie in diesem Beispiel) kann man die Funktion `pairs()` verwenden, um alle paarweisen Streudiagramme gleichzeitig darzustellen:
...
Berechnen Sie abschließend die Pearson-Korrelation zwischen den Daten beider Messstationen inklusive Konfidenzintervall sowie $p$-Wert. Wie viel Varianz der einen Variable kann durch die andere Variable erklärt werden?
...
Berechnen Sie die Korrelation der Schnabellänge (`bill_len`) mit der Schnabeltiefe (`bill_dep`) im Datensatz `penguins`. Berechnen Sie außerdem diese Korrelation für jede Spezies separat! Was stellen Sie fest?
