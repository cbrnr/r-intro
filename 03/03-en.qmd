...
The basic (atomic) data type in R is the vector. A vector is a data structure that consists of one or more elements. Therefore, even a number (a scalar) like `5` is a vector (with only one element).

The `c()` function (stands for combine or concatenate) is used to create a vector. In the following example, we create a vector with four elements and assign the name `y` to it:
...
The previous example could also be written in two lines, but this is of course more typing:
...
The following example creates a vector `a` with a single element – in this case, you don't even need to use the `c()` function:
...
A vector always consists of elements of the same type. Vectors are therefore called *homogeneous* data types. So far, we have learned about numerical vectors, which consist exclusively of numbers. However, there are also vectors that contain logical elements or characters. We can roughly distinguish between the following vector types in R:

- Numeric vectors
- Logical vectors
- Character vectors

In one of the next sessions, we will also learn about factors as a fourth important type (in general, there are many more specific data types in R that we will not cover in this course).
...
The function `class()` can be used to determine the type (class) of an object. Examples:
...
Logical vectors consist exclusively of the values `TRUE` or `FALSE` (pay attention to the correct spelling). They are of type `logical`.
...
:::{.callout-note}
It is possible to abbreviate the values `TRUE` and `FALSE` with `T` and `F`. However, I recommend not using these abbreviations because they are less readable.
:::

Logical vectors are created, among other things, by comparing two vectors:
...
R has the following comparison operators: `>`, `>=`, `<`, `<=`, `==`, and `!=`. Comparisons (or logical vectors) can be combined with `|` (or) and `&` (and) and negated with `!`. Grouping with parentheses is also possible.
...
Character vectors (type `character`) consist of characters enclosed in quotes. Both single `'` and double `"` quotes can be used. A single element of a character vector can consist of one or more letters, numbers, and special characters.
...
The `length()` function returns the length of the vector (i.e., the number of elements) and not the number of characters in a character string. For this, you can use the `nchar()` function:
...
You can also explicitly convert to a desired type with the following functions:

- `as.numeric()`
- `as.logical()`
- `as.character()`

The following example converts a character vector to a numeric vector (this works because in the example all character strings can be interpreted as numbers):
...
We can perform arithmetic operations with numerical vectors – these are always applied *elementwise*:
...
As we already know, the usual operators `+`, `-`, `*`, and `/` are used for addition, subtraction, multiplication, and division. The `^` or `**` symbols calculate the power of a base to an exponent. The operator `%/%` calculates the integer division and `%%` calculates the remainder. Other useful functions are `abs()` for the absolute value and `sqrt()` for the square root of a number. The functions `log()` and `exp()` calculate the (natural) logarithm and the exponential function, respectively. The `sin()` and `cos()` functions calculate the sine and cosine, respectively.
...
The scalar vector `2` is automatically extended to the vector `c(2, 2, 2, 2)`, so the above operation actually corresponds to the following elementwise calculation:
...
The shorter vector `c(0, 10)` is duplicated, and calculation is then performed elementwise:
...
Vectors with defined number sequences can be created with `:` or with `seq()`. In the first option, the step size is always 1, while in the second option, it can be adjusted as desired. The following examples illustrate this:
...
The `seq()` function can be used to create number sequences with a specific step size (`by` argument) or with a specific total length (`length.out` argument).
...
R encodes missing values with the special value `NA` (Not Available). Missing values can occur in any type of vector. The following example creates a vector with missing values:
...
:::{.callout-important}
To find out which elements of a vector are missing, you should not compare with `NA`:
...
Calculate the surface area (base plus lateral surface) and the volume of a cylinder with a radius of 5 and a height of 9. Create the variables `r` and `h` for this purpose. Store the results in the variables `A` (area) and `V` (volume). What are the results (i.e., the values of both variables)?


### Exercise 2

Create a vector `x` with the elements 4, 18, -7, 16, 4, and -44. Then create a vector `y` containing the squared elements of `x` (remember that R performs arithmetic operations elementwise). Finally, create a vector `z` by concatenating `x` and `y`. Which function can you use to determine the number of elements in `z`?
...
Which elements of `x` are even? Which elements are odd? Create two corresponding logical vectors that you can use to index the even and odd elements of `x`.

::: {.callout-note}
Do not create the logical index vectors manually (i.e., for the even numbers in this example, the desired logical index vector is `c(TRUE, FALSE, TRUE, TRUE, FALSE, TRUE, FALSE, TRUE)`), but use a comparison instead. Remember that even numbers have a remainder of 0 when divided by 2 (odd numbers have a remainder of 1). Use the `%%` operator to calculate the remainder of a division. Create a logical index vector with a comparison (for even numbers, compare if the remainder is 0, for odd numbers, compare if the remainder is 1), which you can then use for indexing.
:::
