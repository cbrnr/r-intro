...
In einer der nächsten Einheiten werden wir dann noch Faktoren als vierten wichtigen Typ kennenlernen (grundsätzlich gibt es in R aber noch viele weitere nützliche Datentypen).
...
::: {.callout-note}
Obwohl es technisch gesehen Unterschiede zwischen ganzen Zahlen und Dezimalzahlen gibt, werden beide in R als `numeric` klassifiziert.
:::

### Logische Vektoren

Logische Vektoren bestehen ausschließlich aus den Werten `TRUE` oder `FALSE`. Sie sind vom Typ `logical`.
...
Vergleiche (bzw. logische Vektoren) können mit `|` (oder) und `&` (und) verknüpft und mit `!` negiert (invertiert) werden. Gruppierungen durch Klammersetzung sind ebenfalls möglich.
...
Zeichenvektoren (Typ `character`) bestehen aus Zeichen, welche innerhalb von Anführungszeichen eingegeben werden (es können sowohl einfache `'` als auch doppelte `"` Anführungszeichen verwendet werden). Jedes Element eines Zeichenvektors kann beliebig viele Zeichen enthalten, und zwar eine Kombination aus Buchstaben, Ziffern und Sonderzeichen.
...
Die Funktion `length()` gibt die Länge des Vektors (d.h. die Anzahl der Elemente) und nicht die Anzahl der Zeichen zurück. Dafür kann man die Funktion `nchar()` verwenden:
...
Vektoren sind homogene Datentypen, d.h. sie enthalten nur Elemente desselben Typs. Wenn Elemente unterschiedlicher Typen kombiniert werden, wandelt R sie automatisch in einen Typ um, der alle Werte darstellen kann. Mischt man beispielsweise Zahlen und Zeichen, werden alle Elemente in Zeichen umgewandelt: Zahlen können als Zeichen dargestellt werden, Zeichen jedoch im Allgemeinen nicht als Zahlen.
...
Rechnen kann man mit dem Vektor im obigen Beispiel allerdings nicht mehr, da dessen Elemente nun Zeichen und keine Zahlen mehr sind.
...
Folgendes Beispiel wandelt einen Zeichenvektor in einen numerischen Vektor um – dies funktioniert, weil hier alle Zeichen als Zahlen interpretiert werden können:
...
::: {.callout-note}
Beachten Sie, dass `1:20` bereits einen Vektor erzeugt. Die Funktion `c()` ist hier daher nicht nötig. Man schreibt also einfach `1:20` statt `c(1:20)`.
Im Allgemeinen braucht man `c()` nur, wenn man mehrere vorhandene Objekte oder Werte zu einem Vektor zusammenfügen möchte.
:::

Die Funktion `rep()` wiederholt bestimmte Werte:
...
Vektoren können indiziert werden, d.h. einzelne Elemente können herausgegriffen werden. Man verwendet dazu eckige Klammern, innerhalb derer die gewünschten Elemente angegeben werden. Das erste Element entspricht dem Index 1, das zweite Element dem Index 2, und so weiter.

Betrachten wir in den folgenden Beispielen den Vektor `x`, welcher aus 11 Elementen besteht:

```{r}
(x = c(44, 23, -56, 98, 99, 32, 45, 22, 11, 0, -1))
length(x)
```
...
Man kann auch mit logischen Vektoren indizieren. Dazu erstellt man zuerst einen logischen Vektor (z.B. durch einen Vergleich) und verwendet diesen dann innerhalb der eckigen Klammern (dies kann direkt in einem Schritt gemacht werden). Dabei werden jene Elemente herausgegriffen, für die der logische Vektor `TRUE` ist.
...
```{r}
x = 1:3
names(x)  # x hat noch keine Namen, daher wird NULL zurückgegeben
...
In R können fehlende Werte mit `NA` (Not Available) codiert werden.
...
Erstellen Sie einen Zeichenvektor mit folgenden Einträgen: zuerst 10 mal `"Placebo"`, dann 10 mal `"Group 1"` und schließlich 10 mal `"Group 2"` (d.h. das Ergebnis soll 30 Elemente haben).
